/**
 * Watchdog - ramonnr@kth.se
 * The WD is implemented in hardware, the loadhandler task checks if any thread 
 * is currently unresponsive. If so, it attempts a reset of the thread, before 
 * kicking the WD. If a kick is sent in time, then nemas problemas hombre, 
...
/*
 Fast strcpy function, note that caller is responsible for memory management
*/
...
/*
 Copys all fields from const struct os_thread_def into a user defined struct
...
//loadhandler thread, monitors task activity
